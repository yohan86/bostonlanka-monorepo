  ...
  return (
     <div key={article.slug} className="flex flex-col gap-4 items-center w-full md:w-[50%]">
        <div className="relative w-full h-40  md:h-75 shrink-0">
            <Image 
            src={getImageUrl(article.featuredImage.node.sourceUrl)}
            alt={article.slug} 
            fill 
        ...
        <div>
            ...
            <div className="flex gap-3 items-center my-4">
                ...
                <Link href={`/post/${article.slug}`} aria-label={`view ${article.slug} details`} className="flex gap-2 items-center text-sm hover:gap-3 animate duration-300 text-site-red uppercase">Read more <FaLongArrowAltRight /></Link>
            </div>
        </div>
