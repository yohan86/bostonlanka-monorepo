  ...
  return (
    <div className="flex overflow-hidden shadow-xl items-center bg-white rounded-md">
        ...
        <div className="flex flex-col m-2">
          ...
          <Link href={`/post/${post.slug}`} area-label={`view more ${post.slug} details`} className="flex items-center gap-1 text-[12px] text-site-red">Read more <MdKeyboardDoubleArrowRight size={14} className="mt-0.5" /></Link>
        </div>
    </div>
  )
}
